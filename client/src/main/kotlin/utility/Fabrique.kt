...
class Fabrique(private var scan: Scanner) {
     ...
     * Get scanner
     *
     * @return scanner of the getFabrique
     */
    fun getScan(): Scanner {
        return scan
    }
    ...
    fun askUsername(): String {
        ...
        while (true) {
            try {
                ...
                println("error: Неправильный никнейм. Никнейм должен быть от 3 до 12 и содержать только английские буквы и цифры")
            }
        }
        return username
    }
    ...
    fun askForCommandArguments(str1: String): String {
        if (str1 in listOf("add", "add_if_min", "update")) {
            return askForMovieName()+ " " + askForX() + " " + askForY() + " " + askForOscarsCount() + " " + askForGenre() + " " + askForRating() + " " + askForDirectorName() + " " + askForDirectorBirthday() + " " + askForEyeColor() + " " + askForX() + " " + askForY() + " " + askForZ() + " " + clientManager.getUsername()
        }
